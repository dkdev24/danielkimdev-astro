...
그 작업에는 Wave 오케스트레이션도, 보안 위협 모델러 페르소나도, MCP 서버 자동 라우팅 알고리즘 설명 22KB도 필요하지 않았습니다.
...
AI 에이전트 설정에도 같은 원칙이 적용된다고 생각합니다. 이것이 **최소 구성의 원칙(Minimum Viable Context)**입니다. 각 워크플로는 그 작업을 수행하는 데 필요한 최소한의 컨텍스트만 로드해야 합니다.
...
이 글의 주제는 **설정 방식**입니다. SuperClaude라는 도구 자체를 비판하려는 것이 아닙니다. "한 번 설정하면 어디서나 동작한다"는 편리함이, "필요하지 않은 곳에서도 항상 동작한다"는 낭비로 이어진 것입니다.
