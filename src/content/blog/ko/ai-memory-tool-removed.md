...
- **CHANGELOG.md**: 세션별 전체 기록을 최신 항목이 위에 오도록 쌓아 나갑니다. 자동으로 읽히지 않으며, 더 깊은 컨텍스트가 필요할 때만 참고합니다.
...
- Mem0의 실질적인 강점으로 남아 있던 것은 시맨틱 검색뿐이었는데, `grep`과 CHANGELOG.md 상단을 읽는 것으로 검색이 필요한 경우의 대부분을 충분히 커버했습니다.
...
- **CHANGELOG.md는 제한 없이 커질 수 있습니다.** 자동으로 읽히지 않으므로 에이전트는 필요한 만큼만 읽습니다.

현재 CHANGELOG.md는 4,000줄이 넘습니다. 그러나 문제가 되지 않습니다. 어떤 에이전트도 전체를 컨텍스트에 올리지 않습니다. 최근 세션을 파악하려면 상위 150줄을 읽거나, 특정 주제를 찾으려면 해당 내용을 검색하면 됩니다.
