...
**Last session:** Completed **Stage 09** (content collections & schemas): full content model in `src/content.config.ts` — blog + portfolio (md/mdx) + timeline (json/yaml data) collections with Zod schemas (PRD §9). Two exported tag enums (`BLOG_TAGS` AI-only; `PORTFOLIO_TAGS` media-tech + blog crossover), `PORTFOLIO_CATEGORIES`, and `Lang`/tag types. Locale strategy = folder-by-locale (`{blog,portfolio}/{en,ko}/`) + `lang` field. Added `src/utils/readingTime.ts`. Verified `astro check` 0 errors + off-taxonomy tag fails the build. **Stage 10 (seed content) next.**
...
- **Phase:** Phase 1 (Foundation) complete. **Phase 2 (Content engine) in progress — Stages 01–09 done, Stage 10 next.** PRD finalized; build is broken into executable stages.
...
- **Content model (Stage 09):** `src/content.config.ts` defines **3 collections** — `blog` + `portfolio` (md/mdx) and `timeline` (json/yaml data) — with Zod schemas per PRD §9. **Taxonomy is two separate exported enums:** `BLOG_TAGS` (AI knowledge-work only) and `PORTFOLIO_TAGS` (media-tech `drm-content-security`/`ott-streaming`/`cloud-saas` + the blog tags for crossover) — **import these (and `PORTFOLIO_CATEGORIES`, `Lang`, `BlogTag`, `PortfolioTag`, `PortfolioCategory`) from `content.config.ts`; don't redefine.** Adding an off-enum tag fails the build. **Locale strategy: folder-by-locale + `lang` field** — author blog/portfolio under `src/content/{blog,portfolio}/{en,ko}/`, timeline flat under `src/content/timeline/`, and always set `lang:` in frontmatter. EN/KO counterparts of the same piece share a `translationKey`. `heroImage` is an optimized `image()`; `ogImage` is a plain path string. Reading time: `readingTimeMinutes(body, lang)` in `src/utils/readingTime.ts` → feed into i18n `formatReadingTime`. **GOTCHA:** the blog schema's output type makes defaulted fields (`tags`, `draft`) *required* on the legacy `BlogPost` layout's Props — the legacy `about.astro` now passes `lang`/`tags`/`draft` explicitly (all reworked in Stages 14/16/17).
...
2. **Resume point:** **Stage 10 — seed content** ([`plans/stage-10-seed-content.md`](dev-references/plans/stage-10-seed-content.md)). Author real (or clearly-marked placeholder) EN/KO blog posts, portfolio items, and timeline entries into `src/content/{blog,portfolio}/{en,ko}/` + `src/content/timeline/`, conforming to the Stage 09 schemas. `TODO(daniel)` open: which PKM pieces seed the first 3–5 posts per locale (scaffold with placeholders if undecided). Depends on 09.
