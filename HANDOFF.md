...
**Status:** **LIVE** at https://danielkimdev.com. All P0 (01–21) + P1 (22–27) + P2 (30–32) shipped. Session 18 bumped `@toonstrip/astro` three times (`0.1.6→0.1.7→0.1.8→0.1.9`; see Current state) across two rounds — the first (0.1.7/0.1.8) turned out incomplete, caught only by checking the real deployed screenshots, not by the upstream unit tests. 0.1.9 is the actual fix, verified against the real 363×309 embed size this time. Session 14's homepage pipeline section still not visually verified. Sitewide Hallmark reskin (Cobalt/Aurora, session 13) live. afdocs 99/100 (A); Lighthouse not re-run since 2026-07-01.
...
- **`toonstrip` deployed-blank bug fixed (session 17); balloon-overlap took two rounds (session 18):** session 17 fixed the demo rendering blank in production (CSP `unsafe-eval`, `astro@0.1.6`). Session 18's first round (`0.1.7` balloon-width roll, `0.1.8` `panelSeed` hash fix) looked right in local screenshots but wasn't — the real embed here renders at 363×309, smaller than the upstream renderer's own test fixtures, and its actual failure mode (a different fallback code path) wasn't touched until `astro@0.1.9`. Verified this time by reading the real canvas size off the live page first, then re-screenshotting EN+KO at that exact size — no more overlap in any panel. `dev-references/toonstrip-sync.md` updated with this lesson. Separately, `.prose-breakout` (`src/styles/global.css`) wraps the demo (EN/KO) at a fixed 46rem width since `max-width: 72ch` differs per body font (Pretendard vs Latin), pushing Korean under toonstrip's 2-column threshold.
