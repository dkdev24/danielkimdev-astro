...
**Status:** **LIVE** at https://danielkimdev.com (custom domain; apex + www→apex 301; `.pages.dev` works). All of P0 (Stages 01–21) shipped; **P1 (22–29) and P2 (30–36) fully decomposed** in the index. **22–26 deployed** (Web Analytics · 404 · pagination · blog search · tag archives — last deploy 2026-06-29). **Stage 27 (portfolio detail pages) done locally — NOT yet deployed.** Web Analytics runs via CF **Automatic Setup** (edge beacon). Remaining = deploy Stage 27, post-deploy Lighthouse, a couple of content items, P1 Stages 28–29 — none block launch.
...
2. **Redeploy** — **Stage 27 is built locally and not yet shipped** → `npm run deploy` when ready (predeploy guard prompts on a dirty/non-`main`/drifted tree). *(Last deploy 2026-06-29 = Stages 22–26.)*
