...
**Status:** **LIVE** at https://danielkimdev.com (custom domain; apex + www→apex 301; `.pages.dev` works). All of P0 (Stages 01–21) shipped; **P1 (22–29) fully decomposed**; **P2 stages 30–32 (series, related posts, reading progress) shipped and deployed 2026-07-01**, incl. a post-deploy CSS fix (see below), 33–36 still one-line scope. **Post-deploy Lighthouse done** (live: Perf 97–100, A11y 100, BP 100, SEO 100 — see [`plans/stage-21-deploy.md`](dev-references/plans/stage-21-deploy.md)) — **re-run after this deploy, not yet done for 30–32.** Nothing here blocks launch (already live).
...
- **Shipped (live):** P0 complete + P1 Stages 22–27 + P2 Stages 30–32, all deployed. Last deploy 2026-07-01 (commit `d64f8c8`) — post series, related posts, reading progress, the first `building-llm-pkm-in-public` post, and the `--space-5` CSS fix below. Prod build **59 pages**. Per-stage notes in [`WORKLOG.md`](WORKLOG.md); stage status in [`plans/00-index.md`](dev-references/plans/00-index.md).
...
2. **Redeploy** — `npm run deploy` when new local work lands (predeploy guard prompts on a dirty/non-`main`/drifted tree; guard is Node, not `sh`, for Windows compatibility). Wants an interactive terminal for the `[y/N]` prompt and `wrangler login` if not already authenticated. *(Last deploy 2026-07-01, commit `d64f8c8` — prod in sync with `origin/main`; Daniel deployed and confirmed the CSS fix live.)*
3. **Content** — mostly done. `building-llm-pkm-in-public-ep1` shipped and deployed 2026-07-01; resolved the `portfolio/{en,ko}/digital-garden.md` "link the tooling write-up" TODO by linking to it. Remaining: confirm whatifclassics `period: 2025–2026`. *(Full career timeline 2000–present live; DoveRunner dual role reflected across timeline, bio, and portfolio.)*
