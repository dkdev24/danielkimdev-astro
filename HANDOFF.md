...
- **Not yet deployed:** Stage 30 (post series) + the first `building-llm-pkm-in-public` post built locally 2026-07-01 — local build is **59 pages** (8 more than the deployed 51: series index + agent-readiness hub × EN/KO, series hub for the new series × EN/KO, and the new post itself × EN/KO). Redeploy when ready.
...
4. **P1 stages 28–29** ([`plans/00-index.md`](dev-references/plans/00-index.md)): 28 per-post OG images · 29 authoring docs + content-lint CI. *(22–27 done.)* **P2 stage 30 (series) done** (2026-07-01, not yet deployed — see above); **31 (related posts)** has a plan doc and can be picked up next (its same-series exclusion prerequisite is satisfied); **32 (reading progress)** also has a plan doc, independent of 31. 33–36 still just one-line scope in the index.
