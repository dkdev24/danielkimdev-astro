...
- **Shipped (live):** P0 complete + P1 Stages 22–23. **Built locally, awaiting deploy:** Stages 24 (pagination) + 25 (blog search) + 26 (tag archives). Both locales, all routes 200, `astro check` clean, **e2e 15 passed**, prod build 39 pages. Per-stage notes in [`WORKLOG.md`](WORKLOG.md); stage status in [`plans/00-index.md`](dev-references/plans/00-index.md).
- **Blog index controller** (`BlogIndexPage.astro`): one render pass composes search (substring over `data-search`) AND tag filter AND the pager window. Search box + load-more ship `hidden`, revealed by JS (no-JS shows the full list). **Tag archives** (Stage 26): static `/blog/tags/<tag>/` + `/portfolio/tags/<tag>/` (+ `/ko/`), shared `TagArchivePage.astro` fed by `src/utils/tags.ts`. Portfolio archive cards are **self-contained (no title link)** until Stage 27 adds detail pages. Item tag chips link into the archives.
...
4. **P1 stages 27–29** ([`plans/00-index.md`](dev-references/plans/00-index.md), in order): 27 `/portfolio/[slug]` detail · 28 per-post OG images · 29 authoring docs + content-lint CI. *(22–26 done.)* **After Stage 27**, revisit `src/utils/tags.ts` `getPortfolioTagPaths` + `TagArchivePage` to link portfolio archive card titles to the new detail route (currently plain text by design).
