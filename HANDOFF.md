...
**Last session:** Completed **Stage 06** (base layout shell & landmarks): added `src/layouts/BaseLayout.astro` (canonical shell — `<html lang>`, `BaseHead`+no-flash theme, skip link → `#main`, `header`/`main`/`footer` landmarks; header/footer are placeholder named slots for Stage 07/08); added `noindex` prop + hreflang hook to `BaseHead`; added `.container`/`.container--prose` + global `:focus-visible` ring to `global.css`; migrated `index.astro` onto the shell as a placeholder. Verified `astro check` (0 errors) + landmarks/lang/skip-link in `dist`. **Stage 07 (header nav + language & theme toggles) next.**
...
- **Phase:** Early development (Phase 1; **Stages 01–06 done**, Stage 07 next). PRD finalized; build is broken into executable stages.
...
2. **Resume point:** **Stage 07 — header: nav + language & theme toggles** ([`plans/stage-07-header-nav-toggles.md`](dev-references/plans/stage-07-header-nav-toggles.md)). Real `<header>` chrome injected into BaseLayout's `header` slot: nav (i18n labels via Stage 05 dict), language toggle (`getAltLocale`/`getLocalizedPath`), theme toggle (honor the Stage 03 contract: set `localStorage.theme` + `data-theme`, dispatch `theme-change`). Depends on 06 — done.
