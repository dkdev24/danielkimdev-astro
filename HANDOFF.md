...
**Last session:** Completed **Stage 05** (i18n utilities & UI dictionaries): added `src/i18n/en.json` + `ko.json` (UI string dictionaries, identical key shape, nested-by-surface) and `src/i18n/utils.ts` (`Lang` type, `getLangFromUrl`, `useTranslations`→`t(key, params?)` with typed dot-path lookup + `{token}` interpolation + EN fallback, `getLocalizedPath`, `getAltLocale`, `formatDate`, `formatReadingTime`). Key parity enforced at compile time via `ko satisfies typeof en`. Verified `astro check` (0 errors) + runtime values via tsx. **Stage 06 (base layout shell & landmarks) next.**
...
- **Phase:** Early development (Phase 1; **Stages 01–05 done**, Stage 06 next). PRD finalized; build is broken into executable stages.
...
2. **Resume point:** **Stage 06 — base layout shell & landmarks** ([`plans/stage-06-base-layout.md`](dev-references/plans/stage-06-base-layout.md)). Shared page shell with semantic landmarks, `<html lang>` per locale, skip link, theme/font wiring. (Depends on 03, 04, 05 — all done.)
