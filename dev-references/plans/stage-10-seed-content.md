...
## Tasks
- [x] About prose: place EN draft (§13.1) into the About content/data source; add KO `TODO(daniel):` stub (§13.2) — native voice, no MT.
- [x] Positioning one-liners (§13.3) into `consts.ts` or a home-content data file (EN ready; KO `TODO(daniel):`).
- [x] Portfolio entries (§13.4): one file each — Multi-DRM (product), Watermarking/Anti-Piracy (product), Talks/DevRel (talk-writing), this digital-garden/AI experiments (side-ai), Career timeline (career). Fill known fields; `TODO(daniel):` for metrics/specifics. Mark 3 as `featured: true` for Home.
- [x] Timeline entries: map the 11+yr dev → PO/PM/DevRel arc into `timeline/` data (`TODO(daniel):` exact dates/orgs).
- [x] One sample blog post per locale (`blog/en/`, `blog/ko/`) with full valid frontmatter to exercise the schema and post layout; one demonstrates `translationKey` linkage.
- [x] Ensure at least one post per locale is non-draft so indexes aren't empty.
