...
## Tasks
- [x] Rewrite `src/content.config.ts` blog schema (§9.1): `title, description, pubDate, updatedDate?, lang, tags[], draft(default false), translationKey?, heroImage?, ogImage?`.
- [x] Add `portfolio` collection (§9.2): `title, role, org?, period, summary, category(enum), tags[], lang, links[]?, thumbnail?, featured(default false), order?`.
- [x] Add `timeline` data collection (§9.3): `role, org, start, end, summary, lang`.
- [x] Constrain tags to the **split** taxonomy (§5, positioning locked 2026-06-27) — define two enums:
...
- [x] Create folder structure: `src/content/blog/{en,ko}/`, `src/content/portfolio/{en,ko}/`, `src/content/timeline/`.
- [x] Add a reading-time utility (compute at build from body) for use by index/post stages.
